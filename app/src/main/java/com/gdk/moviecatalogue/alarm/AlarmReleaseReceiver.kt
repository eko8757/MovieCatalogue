...
class AlarmReleaseReceiver : BroadcastReceiver(){
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        context?.let {
            val notificationBuilder = NotificationCompat.Builder(it, StaticString.CHANNEL_ID)
            val titleMovie: String? = it.resources?.getString(R.string.release_today_title)
            var extra : String? = intent?.getStringExtra(StaticString.DATA_MOVIE).toString()
            if (extra == "") {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                val channel = NotificationChannel(StaticString.CHANNEL_ID, StaticString.CHANNEL_NAME, NotificationManager.IMPORTANCE_DEFAULT)
